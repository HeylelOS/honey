...
A directory entry/package name must follow the regular expression: `[^\./][^-/]+-[^/]+`.

### Geist name form

A symbolic link/geist must reference another entry in the prefix, which means it holds
either a directory name or an other geist's name, it must follow the regular expression: `[^\./][^-/]+`.
which supposes every version-differentiating packages of one geist are compatible one another.
